...
int main(int argc, char** argv) {
    ...
    if (argc == 2 && std::string(argv[1]) == "-s") {
        ...
        std::cerr << "./yolov4-tiny -d ../../test_vid.mp4  // deserialize plan file and run inference" << std::endl;
        return -1;
    }
	...
	for (int i = 0; i < INPUT_H * INPUT_W; i++) {
                data[i] = pr_img.at<cv::Vec3b>(i)[2] / 255.0; //R
                data[i + INPUT_H * INPUT_W] = pr_img.at<cv::Vec3b>(i)[1] / 255.0; //G
                data[i + 2 * INPUT_H * INPUT_W] = pr_img.at<cv::Vec3b>(i)[0] / 255.0; //B
        }
